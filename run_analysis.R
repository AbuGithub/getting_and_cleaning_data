...
train_df = cbind(train.data, "act_id" = train.label$V1, "subj_id" = train.subj$V1)
...
test_df =  cbind(test.data, "act_id" = test.label$V1, "subj_id"= test.subj$V1)
...
#identifying position of varaibles with mean() and Std()
#-------------------------------------------------------------
...
df_sel = df_merged[, v]
df_sel = cbind(df_sel, "activity" = df_merged$"act_id", "subject_id"= df_merged$"subject_id")
...
df_sel$subject_id = as.factor(df_sel$subject_id)


#reshaping the df_sel data frame for concise summary
#----------------------------------------------------
print("reshaping data for concise presentation...")
df_melt = melt(df_sel, c("activity", "subject_id"))
df_final = dcast(df_melt, subject_id + activity ~variable, mean)

#change the column labels with tidy labels
#------------------------------------------
tidy_label = c("subjectId", "activity",                   
...
#assign tidy labels to the final data frame
#-------------------------------------------

names(df_final) <- tidy_label
